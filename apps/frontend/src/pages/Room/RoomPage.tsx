...
export default function RoomsPage() {
  ...
  const { isConnected, createRoom, joinRoom, connect } =
    useRoomWebSocket({
      ...
      onJoinApproved: (data) => {
        ...
        setIsJoining(false);
        // Navigate to participant arena
        navigate(`/room/${data.roomId}/arena`);
      },
  ...
  const handleCreateRoom = async (e: React.FormEvent) => {
    ...
    // Connect if not already connected, then create room
    if (!isConnected) {
      connect();
      // Wait for connection to establish before creating room
      await new Promise((resolve) => setTimeout(resolve, 1000));
    }
    console.log("Questions ", selectedProblem)
    const success = createRoom(token, {
      roomname: roomName,
      maxGuests: parseInt(maxParticipants),
    ...
    if (!success) {
      setIsCreating(false);
      toast.error("Failed to send create room request");
    }
  };
  ...
  const handleJoinRoom = async (e: React.FormEvent) => {
    ...
    // Connect if not already connected, then join room
    if (!isConnected) {
      connect();
    ...
    const success = joinRoom(token, roomCode);
